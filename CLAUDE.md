...
npm run docs       # serve the Hugo docs site from docs/
```
...
- **Changelog**: add entries to [changelog.json](changelog.json) (newest version object below the `template` entry, `id` = GitHub issue number or `null`). The husky `pre-commit` hook regenerates `CHANGELOG.md` and `docs/content/changelog/_index.md` from it, and resets the debug shebang in `bin/doctor`.
