...
- Fix: a link with an anchor now resolves to the page it points at. `[text](./page.md#section)` was read as an extensionless link and turned into `./page.md#section.md`, so it matched no file: the link was left as a relative markdown path in the published page — a dead link on the site — and renaming the target did not mark the linking page as changed. The anchor is put back on the rewritten URL.
- Fix: a web part somebody added in SharePoint is no longer claimed because its title matches `--webPartTitle`. Once Doctor knows which controls are its own by instance id, the title is no longer used to guess; it stays the fallback for a page published before those ids were recorded.
- Fix: a control shortcode can use a web part which declares no preconfigured entry, by returning the instance data itself with `webPartData`. That is what the documentation says, but the missing defaults were reported before `webPartData` was looked at, so it could never work.
- Fix: a publish setting that changes is now folded into every page's own hash rather than recorded once for the run. The run-wide record was saved after the first page, so a run which stopped half way left the new settings recorded while the pages it never reached still matched their old hashes — and the next run skipped them for good.
- Fix: toggling `--disableComments` republishes the pages which already exist. It is written onto the page when it is created, but it was not part of what marks a page as changed.
- Fix: a permission check which cannot be read is no longer treated as a refusal. A timeout while reading the Site Pages permissions reported that the account cannot publish and stopped the run, although an unreadable answer is supposed to leave every step attempted, as it already did for the asset library.
- Fix: `--skipPages` runs no longer require permission to write pages. Such a run is there to set the navigation or the site design and does not touch the Site Pages library.
