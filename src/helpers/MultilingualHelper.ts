...
export class MultilingualHelper {
    ...
    for (const locale of localizations) {
      ...
      if (pageName) {
        ...
        if (await existsAsync(localePath)) {
          ...
          Logger.debug(
            `The referenced ${locale} localization page cannot be found (value: "${localePath}").`
          );
          return;
        }
      } else {
        if (options.multilingual && options.multilingual.translator) {
          const {
            translator: { key, endpoint, region },
  ...
  private static async getTranslatedPage(
    ...
    if (!translatedPage) {
      const translationData: PageLocalizationCreation = await ApiHelper.post(
        `${url}_api/sitepages/pages/GetByUrl('sitepages/${encodeURIComponent(
          slug
        )}')/translations/create`,
        {
