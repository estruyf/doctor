...
> **Important**: Arguments always win over the values defined in the `doctor.json` file. A couple of flags can **only** be passed as an argument. These are marked in the [CLI options](./cli-options) reference.
...
> **Info**: Add the `$schema` property to get autocompletion and validation of the configuration file in editors like Visual Studio Code.
