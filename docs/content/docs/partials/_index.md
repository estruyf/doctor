---
title: Partials
date: 2026-08-05T10:06:07.167Z
lastmod: 2026-08-05T10:06:07.167Z
weight: 8
draft: false
keywords:
  - ""
...
> **Info**: A partial is a plain markdown file. It may contain front matter, so it stays a valid markdown file in your editor, but that front matter is not used.
...
> **Info**: `include` tags inside code blocks are left untouched, so you can document them on your pages.
...
> **Info**: The opt-out only applies to the partials which get added to every page. The ones you place yourself with an `include` tag are always added.
...
> **Info**: A link to `.` keeps pointing at the page the partial is included on. Links to other sites are left untouched.
...
- A page is republished when one of the partials it uses has changed. The [`doctor status`](../commands/#status) command shows those pages as modified as well.

> **Important**: Store your partials outside of your sources folder, or `Doctor` will publish them as pages. When they do live inside it, `Doctor` excludes the configured `partials.folder` from the pages it picks up.
