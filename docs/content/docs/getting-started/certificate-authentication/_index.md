...
draft: false
aliases:
  - /docs/certificate-authentication/
keywords:
  - ""
---
...
> **Important**: Since v2.0.0 the `deviceCode` and `password` authentication types are removed. Certificate authentication uses application permissions, which work for every API `doctor` calls, and it is the only type which can run unattended in a CI/CD pipeline.
