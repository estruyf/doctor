...
> **Important**: The `menu` property in the `doctor.json` file is what enables the navigation. When it is not present, the page level `menu` front matter is ignored and no navigation is created. If you only want a page-driven navigation, add an empty definition to your `doctor.json` file:
>
> ```json
> {
>   "menu": {
...
- **`TopNavigationBar`**: the horizontal navigation at the top of your site. How many levels are visible depends on the site its header configuration. The mega menu shows three levels, the cascading navigation shows two. Check the [`disableMegaMenu`](../options/#site-look-and-feel) option to switch between both.
...
> **Info**: Pages with `draft: true` are never added to the navigation.
...
> **Note**: The page level definition takes a **single item** underneath the location, the `doctor.json` definition takes an **array of items** underneath the `items` property. That difference is easy to overlook when you copy an example from one to the other.
...
- [`--cleanQuickLaunch`](../options/#publish-command-specific-options): removes all Quick Launch nodes first.
- [`--cleanTopNavigation`](../options/#publish-command-specific-options): removes all top navigation nodes first.

With [`--skipNavigation`](../options/#publish-command-specific-options) you skip the navigation step completely. Handy when you only want to push content changes, as it also saves you a couple of calls to SharePoint.
...
Check the [change detection](../options/#change-detection--publish-state) section for more information about the publish state.
