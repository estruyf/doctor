...
- Fix: a publish no longer fails when the account is not allowed to change the site navigation. Managing navigation needs rights on the web which an account allowed to edit pages does not necessarily have, and the failure came after every page had already been published. Doctor now reports it and leaves the menu as it is.
