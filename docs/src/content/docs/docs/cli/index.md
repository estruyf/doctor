...
:::note[Info]
When you run `doctor` without any command, it asks you which command you want to execute.
:::
...
:::note[Info]
Existing files are never overwritten, so it is safe to run `doctor init` again in an existing project.
:::
...
:::note[Info]
When state persistence is disabled with `--disableStatePersistence`, no state gets loaded and all pages are reported as **New**.
:::
...
:::note[Info]
An existing `.github/workflows/doctor.yml` or `azure-pipelines.yml` file is never overwritten. Delete or rename it when you want to generate a new one.
:::

:::caution[Important]
The generated file is a starting point. Change the branch, the path filter, or the arguments of the `doctor publish` command to match the way you work.
:::
...
:::caution[Important]
You do not need to use it if you never ran the `doctor setup` command.
:::
