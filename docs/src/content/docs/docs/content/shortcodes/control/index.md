...
- **The web part has to be deployed to the site.** `Doctor` reads its definition from SharePoint to build a new instance from, so a `webPartId` the site does not offer is reported by name. A web part whose definition carries no preconfigured entry has no defaults to start from, and can still be used — return the whole instance yourself with `webPartData`, and `Doctor` only reports it when you have not.
